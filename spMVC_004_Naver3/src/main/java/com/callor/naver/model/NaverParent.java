...
/*
 * Spring 3.x에서 새롭게 추가된 RestTemplate클래스를 사용하여 JSON으로 응답받은 데이터를 한번에
 * VO리스트로 변환하기 위한 Wrapper클래스 선언 
 * VO를 감싸고 있는 클래스
 * 
 * parent 클래스가 VO리스트를 한꺼번에 감싸고 있는 계층구조
 * 데이터 선언 
 * Spring 5.0이상에서는 사용을 자제하라 
 */
...
public class NaverParent<VO> {
	...
	public String display;  
	      /*
	       * naver API를 사용하여 news,Books,Movies정보를 가져오려고 한다
	       * 이때 ResTemplate 를 사용하여 데이터를 가져오는데
	       * RestTemplate은 VO를 감싸는 List변수가 한개 필요하다
	       * 그런데 List를 감싸는 VO가 news,Books,Movies들이 각각 다르다
	       * 이럴때 Parent 클래스를 각각의 VO를 감싸는 타입으로 복수 정의 해야하는
	       * 문제가 있다.       
	       * 이때도 Generic의 기능을 사용하여 클래스 선언문에 Generic을 정의해 두고
	       * 이클래스를 사용할때 확정된 VO를 부착하여 사용한다.
	       */
