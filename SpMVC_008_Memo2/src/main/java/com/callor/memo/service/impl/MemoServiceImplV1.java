...
public class MemoServiceImplV1 implements MemoService {
	...
	public int insertAndUpdate(MemoDTO memo,MultipartFile file) {
		...
		if (m_seq != 0) {
			...
			//table에 저장 하기 위하여 원래 파일 이름과 UUID가 부착된 파일 이름을
			//DTO에 저장
	...
	/*
	 * 1.메모를 삭제할때 첨부파일이 있을경우 파일을 먼저 삭제 해야한다
	 * 2.데이터를 삭제한다.
	 * 
	 * 만약 순서가 바뀌게 되면 첨부파일이 필요없는 파일들이 폴더에 남아있을수있다
	 */
	@Override
	public int delete(Long seq) {
		//seq 에 해당하는 데이터를 select하고
		MemoDTO memo=memoDao.findById(seq);
		
		//파일 이름을 fileDelete()method에게 보내서 파일을 추출하기
