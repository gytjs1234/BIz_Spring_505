...
public class UserServiceImplV2 extends UserServiceImplV1 {
	...
	public UserVO login(UserVO userVO) {
		//controller 에서 전달받은 로그인정보에서 username과 password 를 추출
		...
		//Select 한데이터가 null이면 username 이 없다
		// 잘못입력했거나 아직 회원가입하지 않은 상태
		//Selcet 한 데이터가 null이 아니면 username 있다
		if(joinUserVO !=null) {
			
			boolean bYes=pass.matches(password, joinUserVO.getPassword());
			//비밀번호가 일치하지않으면 null을 return하고 종료하기
			//if(!bYes)
			if(bYes==false) {
				return null;
			}
		}
		//정상 로그인되면 joinUserVO 에는 사용자 정보데이터가 담기고 그렇지 않으면
		//null이 담겨있다.
		return joinUserVO;
	}
