...
/*
 * UserServiceImplV클래스를 Component(bean)로 등록하기
 */
@Service("userServiceV1")
public class UserServiceImplV1 implements UserService{
	
	
	//security-context.xml에 설정된 bean불러오기
	...
	public UserVO login(UserVO userVO) {
		...
		if(userVO.getUsername().equalsIgnoreCase("hyosun")
		&& password.equals("12341234")){
			
			userVO.setName("임효선");
			userVO.setRole("admin");
			userVO.setEmail("dkvkxmdnal@naver.com");
	...
	public UserVO join(UserVO userVO) {
		...
		//1.user table에 데이터가 있는지 확인하기 위하여 전체 데이터를
		//select 하기
