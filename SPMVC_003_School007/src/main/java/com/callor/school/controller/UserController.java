...
public class UserController {
	...
	/*
	 * 1.login,form에서 username과 pssword받기
	 * 2.userVO 에 담겨서 받게 된다
	 * 3.userVO UserService.login에게 전달한다
	 * 4.UserService.login()method는username과 password검사
	 * 5.정상적인(password,password 가 일치)정보이면
	 *    나머지 user 정보를 userVO 에 담아서 return
	 *    만약 정상적인 사용자가 아니면 null을  return
	 * 6.Controller.login.post method에는 HttpSessoin클래스가 매개변수로 설정한다
	 * 7.정상 사용자 정보이면(userVO 가 null이 아니면)
	 *    setAttribute()method를 사용하여 사용자 정보를 변수에 setting한다.
	 * 8.정상사용자가 아니면 removeAttribute()method를 사용하여
	 *    변수를 제거 해 버린다
	 * 
	 */
	...
	public String login(UserVO userVO,HttpSession session,Model model) {
		...
		//로그인 폼에서 입력한 username,password는 
		//userVO에 담겨서 이곳에 도착한다.
		log.debug(userVO.toString());
		
		
		//서버와 뷰 사이에서 약속된 프로토콜을 사용하기 위하여
		//변수를 선언하고  
		String loginMessage=null;
		
		//로그인 폼엥서 전송된 데이터중 username으로 //findBYID()즉 SelectOne(username)
		
		//을 실행한다 그리고 결과를 loginUSERVO에 담는다 만약 username의 정보가 user table에 
		//없으며 결과는 null이고 정보가 있으면 관련데이터가 포함된 VO가 만들어진다
