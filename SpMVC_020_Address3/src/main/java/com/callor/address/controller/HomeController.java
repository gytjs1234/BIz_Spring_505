...
public class HomeController {
	...
	int pageno,SearchPage searchPage) {
	...
	public String update(@RequestParam("seq") long seq, AddressVO adVO) {
		adVO.setA_seq(seq);
		addrService.update(adVO);
	
	
		return "redirect:/detail?seq=" + seq;
	}

	// http://localhost:8080/address/delete?seq=3
	 ...
	 * tiles 에 layout.xml 파일에서 해당하는 문자열로 선언된 definition을 찾는다
	 * 해당하는 문자열로 선언된 definition 이 있으면 layout.xml 설정된 대로
	 * tiles 가 작동되어 layout 을 만들고 rendering 하여 응답을한다
	 * 
	 * 만약 layout.xml 에 해당하는 definition 이 없으면
	 * Internal ResourceViewResolver 가 작동되어 /view/문자열.jsp 파일을 찾아서
	 * redering 하여 응답한다.
	 */
