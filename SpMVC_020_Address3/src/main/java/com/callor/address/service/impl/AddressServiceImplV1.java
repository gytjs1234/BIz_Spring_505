...
public class AddressServiceImplV1 implements AddressService {
	...
	/*
	 * search 문자열이 null 일 경우 전체 데이터를 조회 할 수 있도록 만드는 코드를 Controller 에서 Service 로 이동
	 */
	@Override
	...
	/*
	 * 조건에 맞는 데이터를 가져와서 pageination을 그리기위한 SearchPage 클래스의 데이터를 만들 것
	 */
	@Override
	public void searchAndPage(Model model, SearchPage searchPage) {
		...
		// 1.전체 데이터 개수가 몇개인지를 알아야 pageination 을 구현할수 있다.
		// 화면 하단의 페이지 번호를 자동으로 계산하기 위함이다.
		// 검색어 조건에 맞는 모든 데이터를 일단 select 한다.

		// 검색어가 없이 등장할 경우 SearchPage 객체의 search 값이 null 이되어
		// 데이터가 검색이 되지 않는다
		...
		// 만약 전체 데이터가 37 개이고 한페이지에 10개를 보여준다면
