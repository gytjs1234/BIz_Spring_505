...
public class SendMailServiceImplV2 extends SendMailServiceImplV1 {
	...
	/*
	 * Spring 프로젝트에서 src/main.resource 폴더에 저장된 파일들에 접근하기 위한 보조도구
	 */
	...
	public void sendMail(EmsVO emsVO, UserVO userVO) {
		...
		// 메일을 전송하기 위한 Hepler Class 가져오기(보조적인 도구)메일센더를
		// 도와준다.
		...
		// 네트워크를 통해서 전송하기때문에 try-catch로 묶어주어야한다.
		try {

			/*
			 * 두번째 전달 값 false:순수 text만 전송가능 text/plan type으로 전송 true:html 코드를 포함, 첨부파일도 포함
			 * 가능, text/html, multipart/form-data
			 */
