...
public class UserController {
	 ...
	 * UserVO userVO =new UserVO(); 를대신실행한다.
	 */
	...
	/*
	 * ModelAttribute userVO 코드를 만나면 일단 클라이언트에서 전송되어 userVO 에 담긴 데이터가
	 * 있는지 확인한다 데이터가 있으면 그대로 통과를 하고 만약 데이터가 없으면 SessionAttribute에
	 * 보관중인 userVO데이터를 가져와서 매개변수 userVO에 셋팅한다.
	 * 
	 */
	...
	public String join_next(@Valid @ModelAttribute("userVO") UserVO userVO, BindingResult result, Model model,SessionStatus sessionstatus) {
		...
		/*
		 * log.debug("회원정보 {}", userVO.toString()); return null;
		 */
		}

	/*
	 * 어디선가 UserVO userVO 객체가 필요하면 자동으로 객체를 생성하여
	 *  return 하는 코드가 실행된다.
	 *  
	 *  ModelAttribute로 생성된 userVO 는 SessionAttibute에 추가되어 보관된다.
	 */
